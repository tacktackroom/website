        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {  
                    ...
                    foreach ($order->product_list as $_product) {
                        if ((int)Product::getOfferCount($_product['id_product'], date('Y-m-d'), null, $this->context->cart->id)) {
                            $order->is_offer = 1;
                        }
                    }
                    ...
                    if ($order->is_offer) {
                        ...
                        foreach ($order->product_list as $_product) {
                            if ($offer = Product::getOffer($_product['id_product'], null, date('Y-m-d'), $cart->id)) {
                                $paid_offer = abs($offer['amount']);
                                $_discount  = (float) ($_product['price'] - $paid_offer);

                                $order->total_discounts_tax_excl +=  $_discount;
                                $order->total_discounts_tax_incl += $_discount;
                                $order->total_discounts += $_discount;

                                $order->total_paid_tax_excl = (float) Tools::ps_round($order->total_paid_tax_excl - $_discount, _PS_PRICE_COMPUTE_PRECISION_);
                                
                                $order->total_paid_tax_incl = (float) Tools::ps_round($order->total_paid_tax_incl - $_discount, _PS_PRICE_COMPUTE_PRECISION_);
                            }
                        }

                        $order->total_paid_tax_excl = (float) Tools::ps_round($order->total_paid_tax_excl + $order->total_shipping_tax_excl, _PS_PRICE_COMPUTE_PRECISION_);
                        $order->total_paid_tax_incl = (float) Tools::ps_round($order->total_paid_tax_incl + $order->total_shipping_tax_incl, _PS_PRICE_COMPUTE_PRECISION_);
                        $order->total_paid = $order->total_paid_tax_incl;
                        $order->update();
                    }
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    $totalCartRules = count($cart_rules);

                    foreach ($cart_rules as $cart_rule) {
                        if (!$cart_rule['obj']->quantity || !$cart_rule['obj']->active || 
                            !$order->is_offer) {
                            continue;
                        }
                        ...
                        if ($cart_rule['obj']->product_restriction) {
                            foreach ($order->product_list as $_product) {
                                $cartRuleProductId = (int)Db::getInstance()->getValue('
                                    ...
                                    WHERE cr.`id_cart_rule` = '.(int)$cart_rule['obj']->id .' AND cr.`active` = 1'
                                );

                                if (!$cartRuleProductId || $cartRuleProductId !== (int)$_product['id_product'] ||
                                    !(int)Product::getOfferCount($_product['id_product'], date('Y-m-d'), null, $this->context->cart->id)) {
                                    continue;
                                }

                                if ($cartRuleProductId === (int)$_product['id_product']) {
                                    $values = [
                                        'tax_incl' => $cart_rule['obj']->reduction_amount,
                                        'tax_excl' => $cart_rule['obj']->reduction_amount,
                                    ];
                                } else {
                        ...
                        if (!count($values)) {
                            $values = array(
                                'tax_incl' => $cart_rule['obj']->getContextualValue(true, $this->context, CartRule::FILTER_ACTION_ALL_NOCAP, $package, false),
                                'tax_excl' => $cart_rule['obj']->getContextualValue(false, $this->context, CartRule::FILTER_ACTION_ALL_NOCAP, $package, false),
                            );
                        }
                    ...
                    if (count($cart_rules_list) > 0 && $order->is_offer) {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        '{total_discounts}' => Tools::displayPrice(($order->is_offer ? $order->total_discounts : 0), $this->context->currency, false),
