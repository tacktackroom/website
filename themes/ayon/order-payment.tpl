...
{else}
    <div id="payment_area" class="opc-main-block">
...
<div class="order_frame">
	<h1 class="page-heading">{l s='Please choose your payment method'}</h1>
		<div class="paiement_block">
			<div id="HOOK_TOP_PAYMENT" class="">{$HOOK_TOP_PAYMENT}</div>
				{if $HOOK_PAYMENT}
					{if !$opc}
								...
								<tfoot>
									...
									{else}
										...
										<tr class="cart_total_price">
											...
											<td colspan="2" style="" class="price" id="total_product">{displayPrice price=$total_products}</td>
										</tr>
									{/if}
									...
									<tr class="cart_total_voucher" {if $total_discounts == 0}style="display:none"{/if}>
										<td colspan="4" class="text-right">
											{if $use_taxes}
												{if $priceDisplay}
													{if $display_tax_label}{l s='Total promo codes (tax excl.)'}{else}{l s='Total discount offer'}{/if}
												{else}
													{if $display_tax_label}{l s='Total promo codes (tax incl.)'}{else}{l s='Total discount offer'}{/if}
												{/if}
											{else}
												{l s='Offer'}
											{/if}
									...
									{if $use_taxes}
										...
										<tr class="">
											...
											<td colspan="2" class="price" id="total_price_container">
												<span class="wkbadge label-primary" id="total_price">{displayPrice price=$total_price}</span>
											</td>
										</tr>
									{else}
									<tr class="">
{*
										...
										{/if}
*}
										<td colspan="{if !$voucherAllowed}4{else}4{/if}" class="text-right total_price_container">
{* 											<td colspan="{if !$voucherAllowed}4{else}3{/if}" class="text-right total_price_container"> *}
											<span>{l s='Total'}</span>
										</td>
										<td colspan="2" class="price total_price_container" id="total_price_container">
											<span class="wkbadge label-primary" id="total_price">{displayPrice price=$total_price_without_tax}</span>
										</td>
									</tr>
									{/if}
								</tfoot>
									...
									<tbody>
										{foreach from=$discounts item=discount name=discountLoop}
											<tr class="cart_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">								
												<td class="cart_discount_name" colspan="2">{$discount.name|truncate:30:"...":true}</td>
												<td class="cart_discount_description"></td>
												<td class="cart_discount_price" colspan="3" style="text-align:right; border-right: 2px solid #DB5658;">
					...
					{if $opc}</div></div> <!-- end opc_payment_methods-content -->{/if}
				{else}
