...
$(document).ready(function(){
    ...
    // Wrap width
    if ($('#opc_new_account').hasClass('opc_cont')) {
	    var opcwrap = $("#order-opc #middlecolumns > .container > .row");
	    opcwrap.wrapAll("<div class='opcwrap container'></div>");    	
    }

	// GUEST CHECKOUT / NEW ACCOUNT MANAGEMENT
	if ((typeof isLogged == 'undefined' || !isLogged) || (typeof isGuest !== 'undefined' && isGuest))
	{
		if (guestCheckoutEnabled && !isLogged && !isGuest && !$('#opc_new_account').hasClass('guest_only'))
		{
			$('#opc_account_choice').show();
			$('#opc_account_form, #opc_invoice_address').hide();

				// Add classes for guest form
			    guest_show = $('#opc_new_account').attr('data-guest-show');
				if (guest_show == "1") {
					$('.last_name_main').addClass('last_hide');
				}
				if (guest_show == "2") {
					$('.first_name_main').addClass('name_show_long');
					$('.last_name_main').addClass('last_hide');
				}
				if (guest_show == "3") {
					$('.last_name_main').addClass('last_hide');
					$('.mail_main').addClass('mail_hide');
				}
			...
			$(document).on('click', '#opc_createAccount',function(e){
				...
				$('#opc_account_choice, #opc_invoice_address').hide();
						...
						// Hide guest address
						$('#order-opc .guest_add_opt').hide();
						$('.first_name_main').removeClass('name_show_long');
						$('.last_name_main').removeClass('last_hide');
						$('.mail_main').removeClass('mail_hide');
						$('.first_mob').hide();
						$('.mob_sw').show();

						// Show full address container						
						$('#order-opc .add_cont').css({
					   	'opacity' : '1',
					   	'max-height' : '100%'});
			...
			$(document).on('click', '#opc_guestCheckout',function(e){
				...
				if (guest_show == "3") {
					...
					var erandom = Math.random().toString(36).substr(2, 6),
						emailrandom = 'guest@id'+erandom+'.com';
					$('#email').val(emailrandom);
				}

				$('#order-opc .guest_add_opt').hide();

					// If guest address enabled in customizer - show add address h3
					if ($('#opc_new_account').hasClass('guest_add')) {
						$('#order-opc .guest_add_opt').slideDown('slow');
					} else {
						$('.mob_sw').hide();
					}

					// If guest and normal checkout enabled
					var guest_text = $('#opc_new_account').attr('data-guest-text'),
					    guest_zip = $('#opc_new_account').attr('data-guest-zip');

					$('#order-opc #id_state>option:nth-child(2)').attr('selected', true);
					$('#order-opc #opc_account_form .guest_lastname').val('Guest');
					$('#order-opc #opc_account_form .guest_replace').val(guest_text);
					$('#order-opc #opc_account_form .guest_replace_number').val(guest_zip);

						// If guest address enabled
						$('#order-opc .add_cont').css({
						   	'opacity' : '0',
						   	'max-height' : '0'});

						// When click on add address
						$(document).on('click', '#order-opc .guest_add_opt', function(e){
							$('#order-opc #opc_account_form .guest_replace').val('');
							$('#order-opc #opc_account_form .guest_replace_number').val('');
							$('#order-opc #id_state>option:nth-child(1)').attr('selected', true);
							$('#order-opc .guest_add_opt').hide();
							$('#order-opc .add_cont').css({
						   	'opacity' : '1',
						   	'max-height' : '100%'});
		...
		else if (guestCheckoutEnabled && !isLogged && !isGuest && $('#opc_new_account').hasClass('guest_only'))
		{
		...
		$(document).on('click', '#submitAccount, #submitGuestAccount', function(e){
			...
			$('#order-opc .opc_col_wrapper').removeClass('action_left').addClass('action_right');
