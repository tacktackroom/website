...
{if isset($p) AND $p}
	...
	{elseif isset($smarty.get.mp_shop_name) && $smarty.get.mp_shop_name}
		{assign var='requestPage' value=$link->getPaginationLink('supplier', $supplier, false, false, true, false)}
		{assign var='requestNb' value=$link->getPaginationLink('supplier', $supplier, true, false, false, true)}
	{else}
	...
	{if $nb_products > $products_per_page}
		{if isset($seller_shop) && $seller_shop}
			<form action="{$link->getModuleLink('marketplace', 'shopstore', ['mp_shop_name' => $smarty.get.mp_shop_name])}" method="get" class="nbrItemPage">
		{else}
