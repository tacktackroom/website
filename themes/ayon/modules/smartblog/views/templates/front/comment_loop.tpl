...
<ul class="commentList">
    <div id="comment-{$comment.id_smart_blog_comment}">
                                        <li class="even">
										  ...
										  <div class="options">
                                             <span itemprop="commentTime">{$childcommnets.created|date_format}</span> &nbsp;/ &nbsp;<span itemprop="commentTime">{$childcommnets.created|date_format:"H:i"}</span>
											  {if Configuration::get('smartenablecomment') == 1}
												{if $comment_status == 1} &nbsp;/ &nbsp;<a onclick="return addComment.moveForm('comment-{$comment.id_smart_blog_comment}', '{$comment.id_smart_blog_comment}', 'respond', '{$smarty.get.id_post}')"  class="comment-reply-link">{l s='Reply' mod='smartblog'}</a>
												{/if}
											  {/if}
                                          </div>
