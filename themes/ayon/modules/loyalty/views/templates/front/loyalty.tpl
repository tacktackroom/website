...
<p>{l s='Promo codes generated here are usable in the following categories : ' mod='loyalty'}
{if $categories}{$categories}{else}{l s='All' mod='loyalty'}{/if}</p>

{if $transformation_allowed}
<p class="text-center">
	<a class="btn btn-default" href="{$link->getModuleLink('loyalty', 'default', ['process' => 'transformpoints'], true)|escape:'html':'UTF-8'}" onclick="return confirm('{l s='Are you sure you want to transform your points into promo code?' mod='loyalty' js=1}');">{l s='Transform my points into a promo code of' mod='loyalty'} <span class="price">{convertPrice price=$voucher}</span>.</a>
</p>
{/if}

<h1 class="page-heading">{l s='My promo codes from loyalty points' mod='loyalty'}</h1>
...
{if $minimalLoyalty > 0}<p>{l s='The minimum order amount in order to use these promo codes is:' mod='loyalty'} {convertPrice price=$minimalLoyalty}</p>{/if}

{else}
<p class="alert alert-warning">{l s='No promo codes yet.' mod='loyalty'}</p>
{/if}
{else}
