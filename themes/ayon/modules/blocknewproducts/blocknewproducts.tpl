...
<div id="new-products_block_right" class="block products_block">
    ...
    <div class="block_content products-block">
        {if $new_products !== false}
            <ul id="newul" class="products">
                {foreach from=$new_products item=newproduct name=newLoop}
                    <li class="clearfix {if $smarty.foreach.newLoop.iteration == 1}even{/if}">
                        11<a class="products-block-image" href="{$newproduct.link|escape:'html'}" title="{$newproduct.legend|escape:html:'UTF-8'}"><img class="replace-2x img-responsive" src="{$link->getImageLink($newproduct.link_rewrite, $newproduct.id_image, 'small_default')|escape:'html'}" alt="{$newproduct.name|escape:html:'UTF-8'}" /></a>
            ...
            <div style="text-align: center">
                <a href="{$link->getPageLink('new-products')|escape:'html'}" title="{l s='All new products' mod='blocknewproducts'}" class="btn btn-default button button-small"><span><i class="icon-reorder left"></i>{l s='All new products' mod='blocknewproducts'}</span></a>
            </div>
        {else}
