...
<contact@prestashop.com>
    * @copyright 2007-2014 PrestaShop SA * @license http://opensource.org/licenses/afl-3.0.php Academic Free License (AFL 3.0) * International Registered Trademark & Property of PrestaShop SA *} {if isset($order)} {if isset($reorderingAllowed) && $reorderingAllowed}
    <div class="box box-small clearfix">
        <form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
            ...
            <a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-default button-medium pull-right smaller"><span>{l s='Reorder'}<i class="icon-chevron-right right"></i></span></a>
            <p class="dark">
    ...
    <div class="info-order box">
        {if $carrier->id}
        <p><strong class="dark">{l s='Carrier'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'html':'UTF-8'}{else}{$carrier->name|escape:'html':'UTF-8'}{/if}</p>
        {/if}
    ...
    *}
{*
    <h1 class="xs_title">{l s='Tracking information'}</h1>
    <div class="table_block">
        <table class="detail_step_by_step table table-bordered">
            <tr>
                <th class="first_item">{l s='Tracking number'}</th>
                <td>{if isset($tracking_number) && $tracking_number}{$tracking_number}{/if}</td>
            </tr>
            <tr>
                <th class="last_item">{l s='Tracking url'}</th>
                <td>
                    <a href="{if isset($tracking_url) && $tracking_url}{$tracking_url}{else}javascript:;{/if}" 
                        {if isset($tracking_url) && $tracking_url}target="_blank"{/if}>{l s='Track order'}</a>
                </td>
            </tr>
        </table>
    </div>
*}
    ...
    <h1 class="xs_title">{l s='Follow your order\'s status step-by-step'}</h1>
    <div class="table_block">
        <table class="detail_step_by_step table table-bordered">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
                    ...
                    <td><span class="wkbadge label{if $state.id_order_state == 1 || $state.id_order_state == 10 || $state.id_order_state == 11|| $state.id_order_state == 4||$state.id_order_state == 3} label-info{elseif $state.id_order_state == 5 || $state.id_order_state == 2 || $state.id_order_state == 12} label-success{elseif $state.id_order_state == 6 || $state.id_order_state == 7 || $state.id_order_state == 8} label-danger{elseif $state.id_order_state == 9 } label-warning{/if}">{$state.ostate_name|escape:'html':'UTF-8'}</span></td>
                </tr>
                {/foreach}
            </tbody>
        </table>
    ...
    <a href="{$followup|escape:'html':'UTF-8'}">{$followup|escape:'html':'UTF-8'}</a> {/if}
    <div class="adresses_bloc">
        <div class="row">
            <div class="col-xs-12 col-sm-6 box-small" style="float:left;{if $order->isVirtual()}display:none{/if}">
                <ul class="address_box alternate_item box">
                    <li>
            ...
            <div class="col-xs-12 col-sm-6 box-small" style="float: left">
                <ul class="address_box item {if $order->isVirtual()}full_width{/if} box">
                    <li>
...
{*     {$HOOK_ORDERDETAILDISPLAYED}  *}
    ...
    {if !$is_guest}
    <form action="{$link->getPageLink('order-follow', true)|escape:'html':'UTF-8'}" method="post">
        {/if}
        <div id="order-detail-content" class="table_block table-responsive hidden-xs">
            <table class="table">
                <thead>
                    <tr>
                        {if $return_allowed}
...
{*                         <th class="{if $return_allowed}item{else}first_item{/if}">{l s='Reference'}</th> *}
                ...
                <tfoot>
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed}5{else}3{/if}" style="text-align: right">
                            <strong>{l s='Items (tax excl.)'}</strong>
                        </td>
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                    ...
                    <tr class="item">
                        ...
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                            <span class="medium_number">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
                        </td>
                    </tr>
                    {if $order->total_discounts > 0}
                    <tr class="item">
                        <td colspan="{if $return_allowed}5{else}3{/if}" style="text-align: right">
                            <strong>{l s='Discount'}</strong>
                        </td>
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                    ...
                    <tr class="item">
                        <td colspan="{if $return_allowed}5{else}3{/if}" style="text-align: right">
                            <strong>{l s='Shipping'} {if $use_tax}{l s='(tax incl.)'}{/if} </strong>
                        </td>
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                            <span class="wkbadge wkbadge-pending">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
                        </td>
                    </tr>
                ...
                <tbody>
                    ...
                    <tr class="item">
                        ...
                        <td>
                            <label class="price" for="cb_{$product.id_order_detail|intval}">
                                {if $group_use_tax} {convertPriceWithCurrency price=$product.unit_price_tax_incl currency=$currency} {else} {convertPriceWithCurrency price=$product.unit_price_tax_excl currency=$currency} {/if}
                            </label>
                        </td>
                        <td>
                            <label class="price " for="cb_{$product.id_order_detail|intval}">
                    ...
                    {foreach $product.customizedDatas as $customizationPerAddress} {foreach $customizationPerAddress as $customizationId => $customization}
...
*}
                        <td class="bold">
                            <label for="cb_{$product.id_order_detail|intval}">
                                {if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0} {if isset($is_guest) && $is_guest}
                  ...
                  <img src="{$img_dir}icon/download_product.gif" class="icon" alt="{l s='Download product'}" />
                  </a> {if isset($is_guest) && $is_guest}
                                <a href="{$link->getPageLink('get-file', true, NULL, " key={$product.filename|escape: 'html': 'UTF-8'}-{$product.download_hash|escape: 'html': 'UTF-8'}&id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html':'UTF-8'}" title="{l s='Download this product'}"> {$product.product_name|escape:'html':'UTF-8'} 	</a> {else}
                                <a href="{$link->getPageLink('get-file', true, NULL, " key={$product.filename|escape: 'html': 'UTF-8'}-{$product.download_hash|escape: 'html': 'UTF-8'} ")|escape:'html':'UTF-8'}" title="{l s='Download this product'}"> {$product.product_name|escape:'html':'UTF-8'} 	</a> {/if} {else} {$product.product_name|escape:'html':'UTF-8'} {/if}
                            </label>
                        </td>
                        ...
                        <td class="">
                            <label for="cb_{$product.id_order_detail|intval}">
                                {if $group_use_tax}
                                <span class="">{convertPriceWithCurrency price=$product.unit_price_tax_incl currency=$currency}</span> {else}
                                <span class="">{convertPriceWithCurrency price=$product.unit_price_tax_excl currency=$currency}</span> {/if}
                            </label>
                        </td>
                        <td class="">
                            <label for="cb_{$product.id_order_detail|intval}">
                                {if $group_use_tax}
                                <span class="">{convertPriceWithCurrency price=$product.total_price_tax_incl currency=$currency}</span> {else}
                                <span class="">{convertPriceWithCurrency price=$product.total_price_tax_excl currency=$currency}</span> {/if}
                            </label>
                        </td>
                    </tr>
                    {/if} {/if} {/foreach} 
{*
                    ...
                    <tr class="item">
                        ...
                        <td>{l s='Discount'} {$discount.name|escape:'html':'UTF-8'}</td>
...
*}
        ...
        <div class="container_detail_xs visible-xs effect7" style="display: none">
                    <div class="order-items">
	                 
	                        <div class="col-xs-12" style="text-align: center;margin: 10px 0">
	                           
	                                                         <label class="wkbadge label-primary" style="text-align: center;font-weight: bold"for="cb_{$product.id_order_detail|intval}">
                                {if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0} {if isset($is_guest) && $is_guest}
                                <a href="{$link->getPageLink('get-file', true, NULL, " key={$product.filename|escape: 'html': 'UTF-8'}-{$product.download_hash|escape: 'html': 'UTF-8'}&amp;id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html':'UTF-8'}" title="{l s='Download this product'}">
                  {else}
                  <a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'html':'UTF-8'}-{$product.download_hash|escape:'html':'UTF-8'}")|escape:'html':'UTF-8'}" title="{l s='Download this product'}">
                  ...
                  <img src="{$img_dir}icon/download_product.gif" class="icon" alt="{l s='Download product'}" />
                                ...
                                <a href="{$link->getPageLink('get-file', true, NULL, " key={$product.filename|escape: 'html': 'UTF-8'}-{$product.download_hash|escape: 'html': 'UTF-8'}&id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html':'UTF-8'}" title="{l s='Download this product'}"> {$product.product_name|escape:'html':'UTF-8'} 	</a> {else}
                                <a href="{$link->getPageLink('get-file', true, NULL, " key={$product.filename|escape: 'html': 'UTF-8'}-{$product.download_hash|escape: 'html': 'UTF-8'} ")|escape:'html':'UTF-8'}" title="{l s='Download this product'}"> {$product.product_name|escape:'html':'UTF-8'} 	</a> {/if} {else} {$product.product_name|truncate:35:'...'|escape:'html':'UTF-8'} {/if}
                            </label>   
	                            </div>
{*
...
*}
        ...
        <div class="container_detail_xs visible-xs effect7" style="display: none">
                ...
                <table class="table">
                    {if $priceDisplay && $use_tax}
                    <tr class="item">
                        <td colspan="{if $return_allowed}5{else}4{/if}" style="text-align: left">
                            <strong>{l s='Items (tax excl.)'}</strong>
                        </td>
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}" style="text-align: right">
                            <span class="price">{displayWtPriceWithCurrency price=$order->getTotalProductsWithoutTaxes() currency=$currency}</span>
                        </td>
                    </tr>
                    {/if}
                    <tr class="item">
                        ...
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}" style="text-align: right">
                            <span class="wkbadge label label-success" style="">{displayWtPriceWithCurrency price=$order->getTotalProductsWithTaxes() currency=$currency}</span>
                        </td>
                    </tr>
                    {if $order->total_discounts > 0}
                    <tr class="item">
                        <td colspan="{if $return_allowed}5{else}3{/if}" style="text-align: left">
                            <strong>{l s='Total vouchers'}</strong>
                        </td>
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                            <span class="price-discount" style="text-align: right">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
                        </td>
                    </tr>
                    {/if} {if $order->total_wrapping > 0}
                    <tr class="item">
                        ...
                        <td colspan="{if $order->hasProductReturned()}2{else}1{/if}">
                            <span class="price-wrapping" style="text-align: right">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
                        </td>
                    </tr>
                    {/if}
                    <tr class="item">
...
{/if}
                        ...
                        <br /> {dateFormat date=$message.date_add full=1}
                    </td>
...
{*
    <form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
        <h3 class="xs_title">{l s='Add a message'}</h3>
