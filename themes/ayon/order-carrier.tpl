				...
				{if isset($delivery_option_list)}
					{foreach $delivery_option_list as $id_address => $option_list}
						<p class="carrier_title">
							{l s='In case of you are purchasing multiple items from different sellers, you will be charged for each delivery'}
...
*}<!-- end delivery_options -->
						...
						{foreachelse}
							<p class="alert alert-warning" id="noCarrierWarning">
								{foreach $cart->getDeliveryAddressesWithoutCarriers(true) as $address}
									...
									{else}
										{l s='The seller seems to be in an other country and no carriers available for the address "%s".' sprintf=$address->alias}
										{l s='We recommend you to contact directly the seller and ask him to setup his own carrier.'}
									{/if}
									{if !$address@last}
										<br />
									{/if}
			...
			{if $opc}
				<p class="carrier_title">{l s='Leave a message'}</p>
	 			<div>
	 				<p class="com-desc">{l s='If you would like to add a comment about your order, please write it below.'}</p>
			...
			{/if}
<span style="font-style: italic; float: left;margin-bottom: 30px;width: 100%"><label for="cgv">By clicking button "proceed to checkout" you agree to the terms of service and will adhere to them unconditionally.</label><a href="https://attractivetackroom.com/content/3-terms-and-conditions-of-use?content_only=1" class="iframe" rel="nofollow">(Read the Terms of Service)</a></span>
		{if !$opc}
				<p class="cart_navigation clearfix">
					...
					{if !$is_guest}
						{if $back}
							<a
                                ...
                                <span><i class="icon-chevron-left"></i> {l s='Back'}</span>
							</a>
						{else}
							<a
					...
					{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
						<button type="submit"  onclick="FirePixelEvent('step3')" name="processCarrier" class="button btn btn-default standard-checkout button-medium">
							<span>
