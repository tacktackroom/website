...
$('document').ready( function() {
		...
		var verify = $('#email_create').val();
		var verify2 = $('#email_create2').val();
		if($('#email_create') && (verify != verify2)) {
                        $('#create_account_error').empty();
			$('#create_account_error').show();
			$('#create_account_error').append('<ol><li>{/literal}{l s='The 2 email addresses do not match !'}{literal}</li></ol>');
			return false;
		}
		return true;
	})
...
{if !isset($email_create)}
	...
	<div class="row">
			...
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="create-account_form" class="box">
				...
				<div class="form_content clearfix">
					...
					<div class="form-group wrap-input100">
						...
						<input type="text" class="is_required validate account_input form-control input100" data-validate="isEmail" id="email_create" name="email_create" value="{if isset($smarty.post.email_create)}{$smarty.post.email_create|stripslashes}{/if}" />
						<span class="focus-input100 focus-input100-b" data-placeholder="{l s='Email address'}"></span>
					</div>
					<div class="form-group wrap-input100">
						<label for="email_create2">{l s='Confirm Email address'}</label>
						<input type="text" class="is_required validate account_input form-control input100" data-validate="isEmail" id="email_create2" name="email_create2" onpaste="return false;"/>
						<span class="focus-input100 focus-input100-b" data-placeholder="{l s='Confirm Email address'}"></span>
					</div>
					<div class="submit">
						{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
						<button class="btn btn-default button button-small smaller button_v2" type="submit" id="SubmitCreate" name="SubmitCreate">
							<span>							
		...
		<div class="col-xs-12 col-sm-6 au-login">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
				<h3 class="page-subheading">{l s='Already registered?'}</h3>
				<div class="form_content clearfix">
					<div class="form-group">
						<label for="email">{l s='Email address'}</label>
						<input class="is_required validate account_input form-control" data-validate="isEmail" type="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes}{/if}" placeholder="{l s='Email address'}" />
					</div>
					<div class="form-group">
						<label for="passwd">{l s='Password'}</label>
						<span><input class="is_required validate account_input form-control" type="password" data-validate="isPasswd" id="passwd" name="passwd" value="{if isset($smarty.post.passwd)}{$smarty.post.passwd|stripslashes}{/if}" placeholder="{l s='Password'}" /></span>
					</div>
					<p class="lost_password form-group"><a href="{$link->getPageLink('password')|escape:'html':'UTF-8'}" title="{l s='Recover your forgotten password'}" rel="nofollow">{l s='Forgot your password?'}</a></p>
					<p class="submit">
						{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
						<button type="submit" id="SubmitLogin" name="SubmitLogin" class="btn btn-default button button-small smaller button_v2">
							<span>
        ...
        <div id="oneall_social_login_box 1"></div>
	</div>
...
*}
			<div class="form-group wrap-input100 required col-xs-9 col-md-6 col-lg-6">
				<input onkeyup="$('#firstname').val(this.value);" type="text" class="is_required validate form-control input100" data-validate="isName" id="customer_firstname" name="customer_firstname" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}" />
										<span class="focus-input100" data-placeholder="{l s='First Name' mod='marketplace'}"></span>
			</div>
			<div class="form-group wrap-input100 required col-xs-9 col-md-6 col-lg-6">
				<input onkeyup="$('#lastname').val(this.value);" type="text" class="is_required validate form-control input100" data-validate="isName" id="customer_lastname" name="customer_lastname" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}" />
										<span class="focus-input100" data-placeholder="{l s='Last Name' mod='marketplace'}"></span>
			</div>
            <div class="form-group wrap-input100 required col-xs-9 col-md-6 col-lg-6">                      
                <img style="display: none;" width="25" src="{$modules_dir|escape:'htmlall':'UTF-8'}marketplace/views/img/loading-small.gif" class="seller-loading-img"/>
                <input class="form-control input100 is_required validate"   type="text" value="{if isset($smarty.post.shop_name_unique)}{$smarty.post.shop_name_unique|escape:'htmlall':'UTF-8'}{/if}" 
                    id="shop_name_unique" 
                    data-validate="isName"
                    name="shop_name_unique" 
                    onblur="onblurCheckUniqueshop();"
                    autocomplete="off" />                       
                <span class="focus-input100 required" data-placeholder="{l s='Tackroom name' mod='marketplace'}"></span>
                <span class="wk-msg-shopnameunique"></span>
            </div>
			<div class="form-group wrap-input100 required col-xs-9 col-md-6 col-lg-6">
				<input type="text" class="is_required validate form-control input100 has-val" data-validate="isEmail" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" />
										<span class="focus-input100" data-placeholder="{l s='E-mail' mod='marketplace'}"></span>
			</div>
			<div class="required password form-group wrap-input100 col-xs-9 col-md-6 col-lg-6">
				<input type="password" class="is_required validate form-control input100" data-validate="isPasswd" name="passwd" id="passwd" />
				<span class="focus-input100" data-placeholder="{l s='Password' mod='marketplace'}"></span>
			</div>
			<div class="required password form-group wrap-input100 col-xs-9 col-md-6 col-lg-6">
				<input type="password" class="is_required validate form-control input100" name="confirm_passwd" id="confirm_passwd" />
				<span class="focus-input100" data-placeholder="{l s='Confirmation Password' mod='marketplace'}"></span>
			</div>			
...
{*
			<div class="form-group" style="text-align: center">
				...
				<div style="width: 100%;">
				<div class="row col-xs-12 col-lg-3 col-md-3" style="margin: auto;float: none">
					<label>{l s='Date of Birth'}</label>
					<div class="col-xs-4">
						<select id="months" name="months" class="form-control">
							<option value="">-</option>
...
*}
		...
		{/if}
		{$HOOK_CREATE_ACCOUNT_FORM}
		<div class="submit clearfix" style="text-align: center">
			<input type="hidden" name="email_create" value="1" />
			<input type="hidden" name="is_new_customer" value="1" />
			{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
			<button type="submit" name="submitAccount" id="submitAccount" class="btn btn-default button button-small smaller button_v2">
				<span>{l s='Register'}</i></span>
			</button>
{* 			<p class="pull-right required_pulled_right"><span>{l s='Required field'}</span></p> *}
		</div>
	</form>
{/if}
{strip}
