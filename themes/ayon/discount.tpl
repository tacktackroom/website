...
*}
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{l s='My account'}</a><span class="navigation-pipe">{$navigationPipe}</span><span class="navigation_page">{l s='My promo codes'}</span>{/capture}
...
{else}
	<p class="alert alert-warning">{l s='You do not have any promo codes.'}</p>
{/if}
