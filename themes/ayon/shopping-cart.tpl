	...
	{if isset($lastProductAdded) AND $lastProductAdded}
		<div class="cart_last_product">
			...
			<a class="cart_last_product_img" href="{$link->getProductLink($lastProductAdded.id_product, $lastProductAdded.link_rewrite, $lastProductAdded.category, null, null, $lastProductAdded.id_shop)|escape:'html':'UTF-8'}">
				<img src="{$link->getImageLink($lastProductAdded.link_rewrite, $lastProductAdded.id_image, 'cart_default')|escape:'html':'UTF-8'}" alt="{$lastProductAdded.name|escape:'html':'UTF-8'}"/>
			</a>
	...
	<div id="order-detail-content" class="table_block table-responsive">
		<table id="cart_summary" class="table table-bordered">
			...
			<tfoot>
				{if $use_taxes}
					{if $priceDisplay}
                        <tr class="cart_separator"><td colspan="7"></td></tr>
						<tr>
							...
							{*	{if $voucherAllowed}
									...
									<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											...
											<p>{l s='If you have some promo codes or coupons, you can apply it here.'}</p>
										</fieldset>
									</form>
									{if $displayVouchers}
...
*}
							...
							<td colspan="2" class="price wkbadge label-primary" id="total_product">{displayPrice price=$total_products_wt}</td>
						</tr>
					{/if}
				{else}
                    <tr class="cart_separator"><td colspan="7"></td></tr>
...
{*
								<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
									<fieldset>
										<h4>{l s='Have a Coupon Code?'}</h4>
										<input type="text" class="discount_name form-control" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
										<input type="hidden" name="submitDiscount" />
										<button type="submit" name="submitAddDiscount" class="button btn btn-default button-small">
											<span>{l s='OK'}</span>
										</button>
										<p>{l s='If you have some vouchers, you can type it here to apply price drop for you purchase.'}</p>
									</fieldset>
								</form>
*}
								{if $displayVouchers}
				...
				<tr{if $total_wrapping == 0} style="display: none;"{/if}>
					...
					<td colspan="2" class="price" id="total_wrapping">
						{if $use_taxes}
							{if $priceDisplay}
					...
					{if $use_taxes && $total_shipping_tax_exc != $total_shipping}
						...
						{else}
							<tr class="cart_total_delivery"{if $total_shipping <= 0} style="display:none;"{/if}>
								<td colspan="3" class="text-right">{if $display_tax_label}{l s='Total shipping (tax incl.)'}{else}{l s='Total shipping'}{/if}<span class="notice">{l s='*Shipping costs is provided by the seller based on the package size. If it is too expensive, you can ask the seller to switch to the free shipping option.'}</span></td>
				...
				{/if}
				<tr class="cart_total_price_j">
					<td colspan="2" class="total_price_container text-right">
						<span style="font-size:20px;font-weight: bold!important">{l s='Total'}</span>
					</td>
					{if $use_taxes}
...
<span class="notice">{l s='*Too expensive? Ask the seller to switch to the free shipping option.'}</span>	
<span class="notice_2">{l s='Also, if you are going to purchase 2 items from a SAME seller, ask him to create a bundle and you will not pay the shipping twice!'}</span>	
*}					
						...
						{foreach $customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] as $id_customization=>$customization}
							<tr
								...
								<td class="cart_quantity" colspan="2">
									...
									{else}
									<div class="cart_quantity_button minus clearfix">
											{if $product.minimal_quantity < ($customization.quantity -$quantityDisplayed) OR $product.minimal_quantity <= 1}
												<a
													...
													class="cart_quantity_down btn btn-default"
											...
											{else}
												<a
													...
													class="cart_quantity_down btn btn-default disabled"
													href="#"
													title="{l s='Subtract'}">
													<span><i class="icon-minus"></i></span>
												</a>
										...
										<div class="cart_quantity_button plus clearfix">
											<a
												...
												class="cart_quantity_up btn btn-default"
...
{* 					{$discount|print_r} *}
						<tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
							<td class="cart_discount_name" colspan="3">{$discount.name|truncate:30:"...":true}</td>
{*
...
{*
							<td class="price_discount_del text-center">
								{if strlen($discount.id_cart_rule)}
									<a
										href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_cart_rule}"
			...
			{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
				{if $delivery->id}
					<div class="col-xs-12 col-sm-6"{if !$have_non_virtual_products} style="display: none;"{/if}>
						<ul id="delivery_address" class="address_box item box">
				...
				{if $invoice->id}
					<div class="col-xs-12 col-sm-6">
						<ul id="invoice_address" class="address_box alternate_item box">
			...
			{else}
				{foreach from=$formattedAddresses key=k item=address}
					<div class="col-xs-12 col-sm-6"{if $k == 'delivery' && !$have_non_virtual_products} style="display: none;"{/if}>
						<ul class="address_box {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if} box">
							<li>
	...
	<p class="cart_navigation clearfix">
		...
		<a
			...
			<span><i class="icon-chevron-left"></i> {l s='Continue shopping'}</span>
				...
				{if !$opc}
			<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}" onclick="FirePixelEvent('step1')"{/if}
